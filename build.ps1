...
function Nupkg
{
    ...
    # Build packages
    $nuspec = [IO.Path]::Combine($WorkingDir, 'NuGet', 'SharpHttpServer.csproj.nuspec')
...
function WriteAssemblyVersion
{
    ...
    $Filename = [IO.Path]::Combine($WorkingDir, 'SharpHttpServer', 'Properties', 'AssemblyInfo.cs')
...
$slnFile = [IO.Path]::Combine($WorkingDir, 'SharpHttpServer.sln')
